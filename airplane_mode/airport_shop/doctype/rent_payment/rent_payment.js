...
function init_date_picker(frm)
{
    if (!frm.is_new()) return;
        ...
        const value = frm.doc.for_year && frm.doc.for_month
            ? `${frm.doc.for_year}-${String(frm.doc.for_month).padStart(2, '0')}`
        ...
        $(wrapper).on('change', '#rent_month_picker', function () {
            ...
            frm.set_value('for_month', month+"-"+year );

            frm.refresh_field('for_month');
        });
